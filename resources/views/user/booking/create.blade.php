...
@section('content')
    <div class="container mx-auto px-4 py-6 max-w-2xl">
        <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
            ...
            <div class="p-6">
               <form action="{{ route('user.kamar.booking.store', $room) }}" method="POST">

                    @csrf

                    <div class="mb-4">
                        <label class="block text-gray-700 mb-2">Tanggal Check-in</label>
                        <input type="date" name="tgl_checkin" id="checkin" min="{{ date('Y-m-d') }}"
                    ...
                    <div class="mb-4">
                        ...
                        <input type="date" name="tgl_checkout" id="checkout"
                            min="{{ date('Y-m-d', strtotime('+1 day')) }}" class="w-full px-3 py-2 border rounded-lg"
                            required>
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <div class="flex items-center space-x-2">
                            ...
                            <input type="number" name="jnu_kamar_dipesan" id="jumlahKamar" min="1"
                                max="{{ $room->stok }}" class="w-full px-3 py-2 border rounded-lg text-center"
                                value="1" required>
                        ...
                        <p class="text-sm text-gray-500 mt-1">Stok tersedia: <span
                                id="stockAvailable">{{ $room->stok }}</span> kamar</p>
                    </div>
                    ...
                    <div class="bg-gray-50 p-4 rounded-lg mb-6">
                        ...
                        <div class="flex justify-between mb-1">
                            ...
                            <span class="font-semibold" id="pricePerNight">Rp
                        ...
                        <div class="flex justify-between font-bold">
                            <span>Total:</span>
                            <span id="totalPrice">Rp {{ number_format($room->harga, 0, ',', '.') }}</span>
                        </div>
                    </div>
...
@push('script')
    <script>
        const harga = {{ $room->harga }};
        const stok = {{ $room->jumlah_kamar }};

        function updateTotal() {
            const ci = new Date(checkin.value),
                co = new Date(checkout.value);
